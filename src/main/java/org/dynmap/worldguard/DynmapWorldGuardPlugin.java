...
public class DynmapWorldGuardPlugin extends JavaPlugin {
	...
	private boolean isVisible(String id, String worldname) {
		if ((visible != null) && (visible.size() > 0)) {
			if ((visible.contains(id) == false) && (visible.contains("world:" + worldname) == false) && (visible.contains(worldname + "/" + id) == false)) {
				return false;
			}
		}
	...
	private void addStyle(String resid, String worldid, AreaMarker m, ProtectedRegion region) {
		...
		if ((region.getOwners().getPlayers().size() == 0) && (region.getOwners().getGroups().size() == 0)) {
			unowned = true;
		}
		int sc = 0xFF0000;
		int fc = 0xFF0000;
	...
	private void activate() {
		...
		getServer().getScheduler().scheduleSyncDelayedTask(this, new WorldGuardUpdate(), 40);
